...
const Dashboard: React.FC = () => {
  ...
  // Repay action (with max repay check and UI)
  const handleRepay = async () => {
    if (!lendingPool || !userAddress) return;
    const asset = ASSETS[repayAsset];
    const userDebt = borrows[asset.symbol] || "0";
    const amount = parseUnits(repayAmount, asset.decimals);
    const maxRepay = parseUnits(userDebt, asset.decimals);

    if (!repayAmount || parseFloat(repayAmount) <= 0) {
      setModalError(true);
      setModalTitle("Invalid Repay Amount");
      setModalMessage(ERROR_MESSAGES.zeroAmount);
      setModalOpen(true);
      return;
    }
    if (amount > maxRepay) {
      setModalError(true);
      setModalTitle("Repay Too Much");
      setModalMessage(
        `You cannot repay more than your current debt (${userDebt} ${asset.symbol}).`
      );
      setModalOpen(true);
      return;
    }
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2 items-center">
              <input
                ...
                max={borrows[ASSETS[repayAsset].symbol] || "0"}
              />
              <button
                type="button"
                className="p-2 bg-gray-200 rounded"
                onClick={() => setRepayAmount(borrows[ASSETS[repayAsset].symbol] || "0")}
                disabled={parseFloat(borrows[ASSETS[repayAsset].symbol] || "0") === 0}
              >
                Max
              </button>
            </div>
            <span className="text-xs text-gray-500">
              Max repay: {borrows[ASSETS[repayAsset].symbol] || "0"} {ASSETS[repayAsset].symbol}
            </span>
            <Button
              onClick={handleRepay}
